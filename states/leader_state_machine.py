...
class LeaderStateMachine(Machine):
    ...
    def update(self, lidar_data, lidar_ray_angles, sim_steps):
        ...
        gap_direction = self.leader.sensorsAnalyzer.analyzed_data["positive gap direction"]
        
        if not gap_direction:
            # Pas de gap trouvé, rester à hover
            self.leader.action = [0, 0, 0, 0.1, 0]
            return
