def execute(self):
    ...
    ### Takeoff ###
    if LeaderMachine.takeoff.goingUp in self.configuration:
        distance = 1 - self.position[2]
        self.action[2] = distance
        print("Increasing height")
    if LeaderMachine.takeoff.top in self.configuration:
        self.action = [0,0,0,0,0]
        print("Reached the top !")
        
    ### Height conrol ###
    if LeaderMachine.mission.height.correctingHeight:
        distance = 1 - self.position[2]
        self.action[2] += distance 


    ### Follow the gap ###
    if LeaderMachine.mission.movements.followTheGap.movingForward.moving in self.configuration:
        ...
        if self.previous_active_direction != None:
            print("Previous : " + self.previous_active_direction)
        print("Active : " + self.current_active_direction)
    
    if LeaderMachine.mission.movements.followTheGap.movingForward.centering_in_corner in self.configuration:
        match self.front:
            case "North":
                self.action[0] = self.middle_of_corner[1]+0.1
            case "East":
                self.action[1] = self.middle_of_corner[0]+0.1
            case "South":
                self.action[2] = -1*(self.middle_of_corner[1]+0.1)
            case "West":
                self.action[3] = -1*(self.middle_of_corner[0]+0.1)

    if LeaderMachine.mission.movements.followTheGap.aligning.correctingEastWest in self.configuration:
        self.action[0] += self.dist_wall/10
        print("Correcting East/West center")

    if LeaderMachine.mission.movements.followTheGap.aligning.correctingNorthSouth in self.configuration:
        self.action[1] += self.dist_wall/10
        print("Correcting North/South center")

    ### Waiting follower ###
    if LeaderMachine.mission.movements.waitingFollower in self.configuration:
        self.action = [0,0,0,0,0]
        print(self.message_received)
        if self.follower != None:
            if len(self.follower.leader_message) == 0 or self.follower.leader_message[-1] != "come_closer":
                self.follower.leader_message.append("come_closer")
        if len(self.message_received) > 0 and self.message_received[-1] == "continue moving":
            self.previous_active_direction = self.current_active_direction
            print("going to follow the gap")
            self.send("coucou_trop_bien")

    ### Center in intersection ###
    if LeaderMachine.mission.movements.centerInIntersection.enteringIntersection in self.configuration:
        print("Coucou, plus de deux gaps apparemment : " + str(self.number_of_gaps))
    ...
    if LeaderMachine.mission.movements.centerInIntersection.aligningEastWest.goingEastWest in self.configuration:
    ...
    if LeaderMachine.mission.movements.centerInIntersection.aligningNorthSouth.goingNorthSouth in self.configuration:
    ...
    if LeaderMachine.mission.movements.centerInIntersection.centered in self.configuration:
        self.action = [0,0,0,0,0]
    ...
    ### Align with gap ###
    if LeaderMachine.mission.movements.alignWithGap in self.configuration:
        print("AligningGap")
        if self.current_active_direction != None:
            self.front = self.current_active_direction
        dir = ["West","North","East","South"]
    ...
    ### Entering Gap ###
    if LeaderMachine.mission.movements.enteringGap in self.configuration:
    ...
    # print(self.configuration)
    # if LeaderMachine.mission.movements.followTheGap in self.configuration:
    #     print(self.gaps_dir)
